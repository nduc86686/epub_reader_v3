...
class ContentReader {
  static EpubContentRef parseContentMap(EpubBookRef bookRef) {
    ...
    bookRef.Schema!.Package!.Manifest!.Items!
      ...
      var fileName;
          if(manifestItem.MediaType!.contains("%20")) {
            fileName =Uri.decodeFull('${manifestItem.Href}');
          }else{
        ...
        default:
          var epubByteContentFile = EpubByteContentFileRef(bookRef);
          {
            try {
              ...
              // fileName is a valid URI
              // Proceed with using it
            } catch (e) {
              print('iiiiiiiiiiiiiiiiiiiii: ${e} on file ${fileName}');
              // fileName is not a valid URI
              // Handle the error
            }
